'use strict'
/* globals $, chrome, fetch, bohr, FileReader */

const { Network, TransactionType, Transaction, Key } = require('bohr-js')
...
$('button#importWallet').on('click', function (e) {
  ...
  chrome.storage.local.get('accounts', (result) => {
    ...
    if (importType === 'privateKey') {
      ...
      try {
        key = Bohr.Key.importEncodedPrivateKey(Buffer.from(privateKey, 'hex'))
      } catch (e) {
  ...
  try {
    var tx = new Transaction(
      ...
      Long.fromNumber(txObj.amount), // value
        ...
        // Long.fromNumber(0), // gas toChange
        // Long.fromNumber(0), // gasPrice  toChange
...
async function sendTxToApi (tx) {
  ...
  try {
    const response = await fetch(API + 'broadcast-raw-transaction?raw=' + serialize + '', {
      method: 'GET',
      headers: {
